pub const Clobbers = switch (@import("builtin").cpu.arch) {
    ...
    .mos => packed struct {
        ...
        // and the 16-bit pointer pairs (`Imag16`: `rsN` = `rc(2N)`:`rc(2N+1)`).
        // These are allocator-managed zero-page pseudo-registers and clobbering
        // them has real codegen effect. NOTE: `rc0`/`rc1` (= `rs0`) is the
        // soft-stack pointer and `rc30`/`rc31` (= `rs15`) is the frame pointer;
        // both are reserved and clobbering them is unsafe.
        rc0: bool = false,
        rc1: bool = false,
        rc2: bool = false,
        rc3: bool = false,
